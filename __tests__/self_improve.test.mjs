...
// ── Weekly scope ────────────────────────────────────────────────────────────
{
  ...
  assert.ok(
    typeof r.report.mode_weighted_yield === "number",
    "report has mode_weighted_yield",
  );
  assert.ok(typeof r.report.peak_yield === "number", "report has peak_yield");
  assert.ok(
    typeof r.report.health_score === "number",
    "report has health_score",
  );
  assert.ok(
    Array.isArray(r.report.weekly_snapshots),
    "report has weekly_snapshots",
  );
  assert.ok(r.report.badges, "report has badges");
  assert.ok(
    r.report.badges.earned_this_week,
    "report badges has earned_this_week",
  );
  ...
  assert.strictEqual(
    r.current_cascade.yield_,
    18436.98,
    "weekly yield still correct",
  );
  assert.ok(
    Array.isArray(r.suggestions),
    "weekly scope still returns suggestions",
  );
}
...
// ── Mode detection for different pillar profiles ────────────────────────────
{
  ...
  const r = await callTool("self_improve", {
    text: "10000 100 0 0",
    scope: "daily",
  });
  ...
  assert.ok(
    r.quality_score < 1,
    "DEBUG quality score < 1 (low yield vs expected)",
  );
}
{
...
// ── Privacy boundary: daily modes not in weekly report ─────────────────────
{
  ...
  assert.ok(
    !reportJson.includes("dailyModes"),
    "weekly report does not leak daily modes",
  );
  assert.ok(
    !reportJson.includes('"daily_modes"'),
    "weekly report does not leak daily_modes (snake_case)",
  );
}
...
console.log(
  "✓ self_improve weekly: report artifact with badges + health_score + mode_distribution",
);
console.log(
  "✓ self_improve trend: yield_7d/30d/90d + trajectory + phase_pattern",
);
console.log(
  "✓ self_improve mode detection: DEBUG (high input/low output), IDLE (near-zero)",
);
