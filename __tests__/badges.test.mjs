...
function makeHistory(modes) {
  return modes.map((mode, i) => ({
    ...
    pillars: {
      ...
      output: 1000,
      cacheCreate: 1000,
      cacheRead: mode === "MAINTAIN" ? 100000 : 0,
    },
  }));
}
...
// ── 1. First Spark — 1× leverage ────────────────────────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("first_spark"),
    "First Spark earned at 1× leverage",
  );
}
{
  const pillars = {
  ...
  assert.ok(
    !r.collection.includes("first_spark"),
    "First Spark not earned below 1×",
  );
...
// ── 2. Cascade Engine — 10× leverage ────────────────────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("cascade_engine"),
    "Cascade Engine earned at 10× leverage",
  );
...
// ── 3. Chain Reaction — 100× leverage ───────────────────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("chain_reaction"),
    "Chain Reaction earned at 100× leverage",
  );
  assert.ok(
    r.collection.includes("cascade_engine"),
    "Cascade Engine also earned",
  );
}
...
// ── 4. Foundation — 3 BUILD→MAINTAIN arcs ───────────────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("foundation"),
    "Foundation earned with 3 BUILD→MAINTAIN arcs",
  );
}
{
  // Only 2 arcs
  ...
  assert.ok(
    !r.collection.includes("foundation"),
    "Foundation not earned with 2 arcs",
  );
...
// ── 5. Phoenix — 5× MAINTAIN recovery within 1 day ──────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("phoenix"),
    "Phoenix earned with 5 recoveries",
  );
}
{
  // Only 3 recoveries
  ...
  assert.ok(
    !r.collection.includes("phoenix"),
    "Phoenix not earned with 3 recoveries",
  );
...
// ── 6. Verified — submitted via signed agent ─────────────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("verified"),
    "Verified badge earned when isVerified=true",
  );
}
{
  const pillars = {
  ...
  assert.ok(
    !r.collection.includes("verified"),
    "Verified badge not earned when isVerified=false",
  );
}
...
// ── 7. Cascade Streak — 7 consecutive MAINTAIN days ─────────────────────────
{
  ...
  assert.ok(
    r.collection.includes("cascade_streak"),
    "Cascade Streak earned with 7 consecutive MAINTAIN",
  );
}
{
  ...
  assert.ok(
    !r.collection.includes("cascade_streak"),
    "Cascade Streak broken by DEBUG at end",
  );
  const progress = r.in_progress.find((b) => b.id === "cascade_streak");
  assert.strictEqual(
    progress.progress,
    0,
    "Streak resets to 0 when last day is not MAINTAIN",
  );
}
...
}
{
  ...
  assert.ok(
    !r.collection.includes("top_10"),
    "Top 10 badge not earned at rank 15",
  );
}
{
  const pillars = {
  ...
  assert.ok(
    !r.collection.includes("top_10"),
    "Top 10 badge not earned when rank is null",
  );
}
...
// ── MO§ES: should earn First Spark, Cascade Engine, Chain Reaction ──────────
{
  ...
  assert.ok(
    r.collection.includes("first_spark"),
    "MO§ES: First Spark (2042× leverage)",
  );
...
// ── Return structure ─────────────────────────────────────────────────────────
{
  ...
  assert.ok(
    r.earned_this_week.every((id) => typeof id === "string"),
    "earned_this_week contains IDs",
  );
}
...
console.log(
  "✓ badges: Cascade Streak (7 consecutive MAINTAIN) — earn + progress + reset",
);
console.log("✓ badges: Top 10 (rank ≤ 10) — binary, null-safe");
console.log(
  "✓ badges: MO§ES earns First Spark + Cascade Engine + Chain Reaction + Verified",
);
console.log(
  "✓ badges: return structure (earned_this_week, in_progress, collection)",
);
