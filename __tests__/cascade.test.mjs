 ...
 */
import { cascade, detectMode, qualityScore, MODE_EXPECTED_YIELD, parsePillars } from '../cascade.mjs'
...
{
  const r = detectMode({ input: 200, output: 200, cacheCreate: 200, cacheRead: 200 })
...
// MAINTAIN: high leverage + high velocity (the cascade is compounding)
{
  ...
  const r = detectMode({ input: 100, output: 200, cacheCreate: 500, cacheRead: 10000 })
...
}
{
  ...
  const r = detectMode({ input: 1000, output: 800, cacheCreate: 2000, cacheRead: 4000 })
  assert.strictEqual(r.mode, 'MAINTAIN', 'leverage>3 + velocity>0.5 → MAINTAIN')
...
// DEBUG: low velocity + high input share (reading, not producing)
{
  ...
  const r = detectMode({ input: 10000, output: 100, cacheCreate: 0, cacheRead: 0 })
...
// EDIT: high input share + high velocity (fresh input but producing)
{
  ...
  const r = detectMode({ input: 3000, output: 3000, cacheCreate: 0, cacheRead: 10 })
...
// BUILD: fallback (high input, no cache reuse)
{
  ...
  // → BUILD fallback
  const r = detectMode({ input: 399, output: 601, cacheCreate: 0, cacheRead: 0 })
...
// ── MO§ES data: should be MAINTAIN ──────────────────────────────────────────
{
  ...
  assert.strictEqual(r.mode, 'MAINTAIN', 'MO§ES → MAINTAIN (leverage 2042×, velocity 9.0)')
...
// ── cascade() now includes mode ─────────────────────────────────────────────
{
  ...
  assert.strictEqual(c.mode.mode, 'MAINTAIN', 'cascade mode = MAINTAIN for MO§ES')
...
// ── MODE_EXPECTED_YIELD ─────────────────────────────────────────────────────
{
  ...
  assert.strictEqual(MODE_EXPECTED_YIELD.MAINTAIN, 5000, 'MAINTAIN expected yield')
...
console.log('✓ qualityScore: DEBUG 80%, MAINTAIN 1%, BUILD 80%, IDLE 100%, outperform 400%')
