...
export async function submitSignedWindow(windowKey, pillars, messages, identity, opts = {}) {
  if (!identity?.codename || !identity?.operator_id) {
    return { status: 'not_enrolled', reason: 'not_enrolled', detail: 'Run `npx sigrank-mcp enroll` to bind this device first.' }
  }
  if (!identity?.private_key_pkcs8_b64) {
    return { status: 'error', reason: 'no_key', detail: 'No local signing key — re-run `npx sigrank-mcp enroll`.' }
  }
