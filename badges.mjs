...
export function computeBadges({
  ...
  if (leverage > 1) {
    earned.push({
      ...
      date_earned: _latestDate(history),
    });
    ...
    inProgress.push({
      ...
      label: "First Spark",
      icon: "◈",
      progress: Math.min(leverage, 1),
      target: 1,
      display: `${leverage.toFixed(1)}× / 1×`,
    });
  }
  ...
  if (leverage > 10) {
    ...
    inProgress.push({
      ...
      label: "Cascade Engine",
      icon: "⚡",
      progress: Math.min(leverage, 10),
      target: 10,
      display: `${leverage.toFixed(1)}× / 10×`,
    });
  }
  ...
  if (leverage > 100) {
    ...
    inProgress.push({
      ...
      label: "Chain Reaction",
      icon: "⚡",
      progress: Math.min(leverage, 100),
      target: 100,
      display: `${leverage.toFixed(1)}× / 100×`,
    });
  }
  ...
  if (phoenixCount >= 5) {
    ...
    inProgress.push({
      ...
      label: "Phoenix",
      icon: "🔥",
      progress: phoenixCount,
      target: 5,
      display: `${phoenixCount}/5 recoveries`,
    });
  }
...
function _countPhoenixReturns(history) {
  ...
  for (let idx = 1; idx < history.length; idx++) {
    if (
      history[idx].mode === "MAINTAIN" &&
      history[idx - 1].mode !== "MAINTAIN"
    ) {
      count++;
    }
  }
