...
const CASCADE_OUTPUT = {
  type: 'object',
  properties: {
    yield_: { type: 'number', description: 'Υ Yield — the headline efficiency metric (Cache Reads × Output / Input²)' },
    snr: { type: 'number', description: 'Signal-to-noise ratio' },
    leverage: { type: 'number', description: 'Cr/I — cache reads divided by input' },
    velocity: { type: 'number', description: 'O/I — output divided by input' },
    tenx_dev: { type: 'number', description: '10xDEV score' },
    class: { type: 'string', enum: ['Burner', 'Builder', '10xer'], description: 'Operator class tier' },
    card: { type: 'string', description: 'Deterministic prose summary of the cascade result' },
    warnings: { type: 'array', items: { type: 'string' }, description: 'Parse or data warnings if any' },
  },
...
const SUBMIT_OUTPUT = {
  type: 'object',
  properties: {
    status: { type: 'string', enum: ['ok', 'error', 'skipped'], description: 'Submission status' },
    preview: CASCADE_OUTPUT,
    server_response: { type: 'object', description: 'Server-side response including new rank if accepted' },
    reason: { type: 'string', description: 'Error or skip reason if status is not ok' },
  },
}
    ...
    windows: {
      ...
      items: {
        ...
        properties: {
          ...
          messages: { type: 'integer', description: 'Number of messages in window' },
          estimated: { type: 'boolean', description: 'True if cacheCreate was estimated' },
...
const ENROLL_OUTPUT = {
  type: 'object',
  properties: {
    status: { type: 'string', enum: ['enrolled', 'error'], description: 'Enrollment result' },
  ...
  properties: {
    ...
    sources: {
      ...
      items: {
        type: 'object',
        properties: {
          source: { type: 'string', description: 'Source name (tokenpull, ccusage, token-dash, tokscale)' },
          pillars: { type: 'object' },
          cascade: CASCADE_OUTPUT,
          delta_pct: { type: 'object', description: 'Delta % vs tokenpull baseline' },
        },
      },
    },
  },
...
const SIMULATE_OUTPUT = {
  ...
  properties: {
    ...
    yield_delta: { type: 'number', description: 'Υ Yield change (simulated - current)' },
    class_change: { type: 'string', description: 'Class tier change description' },
    metric_diffs: { type: 'object', description: 'Per-metric before/after diffs' },
  },
}
  ...
  properties: {
    ...
    diagnosis: {
      ...
      items: {
        type: 'object',
        properties: {
          metric: { type: 'string', description: 'Which metric is underperforming' },
          severity: { type: 'string', enum: ['critical', 'warning', 'info'], description: 'How bad the leak is' },
          finding: { type: 'string', description: 'What the analysis found' },
          recommendation: { type: 'string', description: 'What to do about it' },
          estimated_yield_impact: { type: 'string', description: 'Estimated Υ improvement if fixed' },
        },
      },
    },
    summary: { type: 'string', description: 'One-line summary of the operator\'s cascade health' },
  },
}
    ...
    suggestions: {
      ...
      items: {
        ...
        properties: {
          ...
          simulated_yield: { type: 'number', description: 'Projected Υ after the change' },
          yield_delta: { type: 'number', description: 'Υ change vs current' },
          class_after: { type: 'string', description: 'Projected class after the change' },
          rationale: { type: 'string', description: 'Why this helps' },
        },
      },
    },
    current_yield: { type: 'number', description: 'Current Υ before any changes' },
    current_class: { type: 'string', description: 'Current class tier' },
    best_single_change: { type: 'string', description: 'The single highest-impact change' },
  },
}
...
export const TOOLS = [
  {
    ...
    annotations: { ...ANNOTATIONS.readOnlyHint, ...ANNOTATIONS.destructiveHint, ...ANNOTATIONS.idempotentHint, ...ANNOTATIONS.openWorldHint },
    inputSchema: {
      type: 'object',
      properties: {
        text: {
  ...
  },
  {
    ...
    annotations: { ...ANNOTATIONS.destructiveHint, ...ANNOTATIONS.idempotentHint, ...ANNOTATIONS.openWorldHint },
    inputSchema: {
      type: 'object',
      properties: {
        text: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        text: {
          type: 'string',
          description: 'Token pillars — ccusage JSON or "input output cacheCreate cacheRead" (same format as rank_paste).',
        },
      },
      required: ['text'],
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        text: {
          type: 'string',
          description: 'Optional: token pillars — ccusage JSON or "input output cacheCreate cacheRead". If omitted, runs tokenpull to get current pillars from local logs.',
        },
        window: {
          type: 'string',
          enum: ['7d', '30d', '90d', 'all'],
    ...
    outputSchema: {
      type: 'object',
      properties: {
        pillars: { type: 'object', description: 'The 4 raw token pillars used' },
        current_cascade: CASCADE_OUTPUT,
        diagnosis: { type: 'array', description: 'Efficiency leaks found (from diagnose_cascade)' },
...
export async function callTool(name, args, opts = {}) {
  ...
  if (name === 'diagnose_cascade') {
    if (!args?.text) throw new Error('diagnose_cascade requires a non-empty `text` argument (token pillars).')
    if (typeof args.text === 'string' && args.text.length > MAX_INPUT) {
      return { status: 'error', reason: 'input_too_large', detail: `text exceeds ${MAX_INPUT} chars.` }
    }
  ...
  if (name === 'suggest_improvements') {
    if (!args?.text) throw new Error('suggest_improvements requires a non-empty `text` argument (token pillars).')
    if (typeof args.text === 'string' && args.text.length > MAX_INPUT) {
      return { status: 'error', reason: 'input_too_large', detail: `text exceeds ${MAX_INPUT} chars.` }
    }
    ...
    if (cr === 0 && cw === 0) {
      ...
      for (const amt of starterAmounts) {
        const sim = cascade({ ...p, cacheCreate: Math.round(amt * 0.5), cacheRead: amt })
    ...
    const top = candidates.slice(0, 8).map((c, idx) => ({ rank: idx + 1, ...c }))
  ...
  if (name === 'self_improve') {
    ...
    if (args?.text) {
      ...
      try {
        const pullResult = await callTool('tokenpull', { window: windowType }, opts)
        if (pullResult?.status === 'error' || !pullResult?.windows?.length) {
          return {
            status: 'error',
            reason: 'tokenpull_failed',
            detail: 'Could not pull pillars from local logs. Provide pillars via `text` argument instead.',
    ...
    for (const mult of crBoosts) {
      ...
      if (sim.yield !== null) candidates.push({
        action: `Increase cache reads by ${Math.round((mult - 1) * 100)}%`,
        pillar: 'cacheRead', delta: `+${Math.round(cr * (mult - 1)).toLocaleString()}`,
        simulated_yield: sim.yield, yield_delta: Number((sim.yield - (currentResult.yield ?? 0)).toFixed(2)),
        class_after: sim.class, rationale: 'Cache reads are the strongest Υ multiplier.',
      })
    }
    // For zero-cache operators, suggest enabling caching with a starter amount
    if (cr === 0 && cw === 0) {
      const starterAmounts = [Math.round(i * 10), Math.round(i * 50), Math.round(i * 100)]
      for (const amt of starterAmounts) {
        const sim = cascade({ ...pillars, cacheCreate: Math.round(amt * 0.5), cacheRead: amt })
        if (sim.yield !== null && sim.yield > 0) candidates.push({
          action: `Enable caching with ${amt.toLocaleString()} cache reads`,
          pillar: 'cacheRead', delta: `+${amt.toLocaleString()}`,
          simulated_yield: sim.yield, yield_delta: Number((sim.yield - (currentResult.yield ?? 0)).toFixed(2)),
          class_after: sim.class, rationale: 'You have no cache — enabling it unlocks the cascade. Start by reusing prior context.',
        })
      }
    }
    for (const mult of [0.9, 0.75, 0.5]) {
      ...
      if (sim.yield !== null) candidates.push({
        ...
        simulated_yield: sim.yield, yield_delta: Number((sim.yield - (currentResult.yield ?? 0)).toFixed(2)),
        class_after: sim.class, rationale: 'Input is squared in Υ denominator — quadratic payoff.',
      })
    }
    for (const mult of [1.25, 1.5, 2]) {
      ...
      if (sim.yield !== null) candidates.push({
        ...
        simulated_yield: sim.yield, yield_delta: Number((sim.yield - (currentResult.yield ?? 0)).toFixed(2)),
        class_after: sim.class, rationale: 'Output is a linear multiplier in Υ.',
      })
    }
    candidates.sort((a, b) => b.yield_delta - a.yield_delta)
    const suggestions = candidates.slice(0, 8).map((c, idx) => ({ rank: idx + 1, ...c }))
    ...
    if (best) {
      ...
      else if (best.pillar === 'input') simPillars.input = Math.max(i - delta, 1)
