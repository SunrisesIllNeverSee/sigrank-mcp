...
async function _tokscaleDetectClients() {
  ...
  // Complete tokscale client → our platform name mapping.
  // Tokscale supports 40 clients (as of v3.1.0). We have native adapters for 16;
  // the rest map to "other" (user-supplied JSON via SIGRANK_OTHER_PATH).
  ...
  const TOKSCALE_CLIENT_MAP = {
    ...
    codebuddy: "other",
    workbuddy: "other",
    // synthetic = re-attributed from other sources, skip
    synthetic: null,
  };
