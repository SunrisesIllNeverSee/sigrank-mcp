...
import { canonicalJson, snapshotHash, signPayload, verifyPayload, canonicalBytes } from './sign.mjs'
import { generateIdentity } from './keystore.mjs'

const fx = JSON.parse(readFileSync(new URL('./tests/fixtures/canon_parity.json', import.meta.url), 'utf-8'))

// 1. canonicalJson is byte-stable vs the committed fixture.
assert.strictEqual(canonicalJson(fx.payload), fx.expected_canonical, 'canonical JSON drift vs fixture')

// 2. snapshot_hash reproduces.
assert.strictEqual(snapshotHash(fx.payload), fx.expected_snapshot_hash, 'snapshot_hash drift vs fixture')
...
assert.ok(verifyPayload(fx.payload, sig, fx.public_key), 'verify must accept a freshly-signed payload')
...
assert.ok(!verifyPayload(tampered, sig, fx.public_key), 'verify must reject a tampered payload')
...
assert.strictEqual(canonicalJson(stripped), fx.expected_canonical, 'canonical must ignore derived agent fields')
...
assert.ok(verifyPayload(fx.payload, s2, id.public_key), 'generated keypair must sign→verify')
assert.ok(id.public_key.startsWith('ed25519:'), 'public key carries the ed25519: prefix')
assert.strictEqual(Buffer.from(id.public_key.slice('ed25519:'.length), 'base64').length, 32, 'raw public key is 32 bytes')

console.log(`✓ sign.test.mjs — canon parity + ed25519 round-trip (canonical ${canonicalBytes(fx.payload).length} bytes)`)
