...
The ingest system emits integrity reasons separately from numerical signals; a flag is a request for caution, not an accusation.

> **Repo scope.** The ingest system (`lib/ingest/gates.ts`) runs **server-side** in `sigrank-app`. The MCP client does not emit integrity reasons; it submits signed snapshots and surfaces the server's `verification_tier` in the submit ack.

Source (server): `sigrank-app/lib/ingest/gates.ts`.